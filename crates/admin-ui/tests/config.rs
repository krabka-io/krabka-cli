...
fn default_config_targets_local_server_and_requires_bootstrap() {
    ...
    assert!(cfg.listen_addr == "127.0.0.1:8088".parse::<SocketAddr>().unwrap());
    assert!(cfg.cluster_name == "local");
...
fn mutation_json_body_limit_default_and_boundaries_are_dimensioned() {
    ...
    assert!(cfg.mutation_json_body_limit == mebibytes(1));
...
fn mutation_json_body_limit_environment_and_cli_precedence() {
    let output = Command::new(std::env::current_exe().expect("test binary path is available"))
        ...
        .env("KRABKA_ADMIN_UI_MUTATION_JSON_BODY_LIMIT", "32B")
        .output()
...
fn mutation_json_body_limit_precedence_child() {
    ...
    assert!(from_env.mutation_json_body_limit == bytes(32));
    ...
    assert!(from_cli.mutation_json_body_limit == bytes(64));
}
...
fn session_ttl_default_remains_eight_hours() {
    ...
    assert!(cfg.session_ttl == hours(8));
}
...
fn session_ttl_rejects_invalid_values() {
    for invalid in ["0s", "not-a-number", "-1s", "1"] {
        assert!(
            AdminUiRuntimeArgs::try_parse_from(["krabka-admin-ui", "--session-ttl", invalid])
                .is_err(),
            "{invalid:?} must be rejected"
        );
    }
...
fn session_ttl_environment_and_cli_precedence() {
    let output = Command::new(std::env::current_exe().expect("test binary path is available"))
        ...
        .env("KRABKA_ADMIN_UI_SESSION_TTL", "32s")
        .output()
        .expect("child test process runs");
...
fn session_ttl_precedence_child() {
    ...
    let from_env = AdminUiRuntimeArgs::try_parse_from(["krabka-admin-ui"])
        .expect("environment value is valid");
    assert!(from_env.session_ttl == secs(32));

    let from_cli = AdminUiRuntimeArgs::try_parse_from(["krabka-admin-ui", "--session-ttl", "64s"])
        .expect("CLI value is valid");
    assert!(from_cli.session_ttl == secs(64));
}
...
fn topic_mutation_timeout_default_remains_thirty_seconds() {
    ...
    assert!(cfg.topic_mutation_timeout == secs(30));
}
...
fn topic_mutation_timeout_environment_and_cli_precedence() {
    let output = Command::new(std::env::current_exe().expect("test binary path is available"))
        ...
        .env("KRABKA_ADMIN_UI_TOPIC_MUTATION_TIMEOUT", "32ms")
        .output()
        .expect("child test process runs");
...
fn topic_mutation_timeout_precedence_child() {
    ...
    let from_env = AdminUiRuntimeArgs::try_parse_from(["krabka-admin-ui"])
        .expect("environment value is valid");
    assert!(from_env.topic_mutation_timeout == millis(32));

    let from_cli =
        AdminUiRuntimeArgs::try_parse_from(["krabka-admin-ui", "--topic-mutation-timeout", "64ms"])
            .expect("CLI value is valid");
    assert!(from_cli.topic_mutation_timeout == millis(64));
}
...
fn validates_single_cluster_sasl_plaintext_config() {
    ...
    assert!(validated.cluster_name == "dev");
    assert!(validated.bootstrap_addrs == ["127.0.0.1:9092"]);
}
...
fn from_env_parses_sasl_ssl_tls_config() {
    let output = Command::new(std::env::current_exe().expect("test binary path is available"))
        ...
        .arg("--nocapture")
        .env("KRABKA_ADMIN_UI_CONFIG_SASL_SSL_CHILD", "1")
        .env("KRABKA_ADMIN_UI_LISTEN_ADDR", "127.0.0.1:18088")
        .env("KRABKA_ADMIN_UI_CLUSTER_NAME", "staging")
        .env("KRABKA_ADMIN_UI_BOOTSTRAP", "broker-1:9092, 127.0.0.1:9093")
        .env("KRABKA_ADMIN_UI_SECURITY_PROTOCOL", "SASL_SSL")
        .env("KRABKA_ADMIN_UI_TLS_TRUST_ROOTS_PEM", "ca.pem")
        .env("KRABKA_ADMIN_UI_TLS_SERVER_NAME", "broker.example.test")
        .env("KRABKA_ADMIN_UI_TLS_CLIENT_CERT_PEM", "client.crt")
        .env("KRABKA_ADMIN_UI_TLS_CLIENT_KEY_PEM", "client.key")
        .output()
        .expect("child test process runs");
...
fn sasl_ssl_tls_config_from_env_child() {
    if std::env::var_os("KRABKA_ADMIN_UI_CONFIG_SASL_SSL_CHILD").is_none() {
        return;
    }

    let cfg = AdminUiConfig::from_env().expect("env config is valid");
    assert!(cfg.listen_addr == "127.0.0.1:18088".parse::<SocketAddr>().unwrap());
    assert!(cfg.cluster_name == "staging");
    assert!(cfg.bootstrap_addrs == ["broker-1:9092", "127.0.0.1:9093"]);
    assert!(
        cfg.security
...
fn from_env_rejects_blank_bootstrap_entry() {
    let output = Command::new(std::env::current_exe().expect("test binary path is available"))
        ...
        .arg("--nocapture")
        .env("KRABKA_ADMIN_UI_CONFIG_BLANK_BOOTSTRAP_CHILD", "1")
        .env("KRABKA_ADMIN_UI_BOOTSTRAP", "broker-1:9092,,broker-2:9093")
        .env("KRABKA_ADMIN_UI_SECURITY_PROTOCOL", "SASL_PLAINTEXT")
        .env_remove("KRABKA_ADMIN_UI_LISTEN_ADDR")
        .output()
        .expect("child test process runs");
...
fn blank_bootstrap_entry_from_env_child() {
    if std::env::var_os("KRABKA_ADMIN_UI_CONFIG_BLANK_BOOTSTRAP_CHILD").is_none() {
        return;
    }
...
fn validates_multi_bootstrap_hostnames_and_ip_literals() {
    ...
    assert!(validated.bootstrap_addrs == ["broker-1:9092", "127.0.0.1:9093"]);
}
...
fn from_env_rejects_empty_tls_server_name_for_sasl_ssl() {
    let output = Command::new(std::env::current_exe().expect("test binary path is available"))
        ...
        .arg("--nocapture")
        .env("KRABKA_ADMIN_UI_CONFIG_EMPTY_TLS_CHILD", "1")
        .env("KRABKA_ADMIN_UI_BOOTSTRAP", "127.0.0.1:9092")
        .env("KRABKA_ADMIN_UI_SECURITY_PROTOCOL", "SASL_SSL")
        .env("KRABKA_ADMIN_UI_TLS_SERVER_NAME", "")
        .env_remove("KRABKA_ADMIN_UI_LISTEN_ADDR")
        .output()
        .expect("child test process runs");
...
fn empty_tls_server_name_from_env_child() {
    if std::env::var_os("KRABKA_ADMIN_UI_CONFIG_EMPTY_TLS_CHILD").is_none() {
        return;
    }
...
fn from_env_rejects_whitespace_tls_server_name_for_sasl_ssl() {
    let output = Command::new(std::env::current_exe().expect("test binary path is available"))
        ...
        .arg("--nocapture")
        .env("KRABKA_ADMIN_UI_CONFIG_WHITESPACE_TLS_CHILD", "1")
        .env("KRABKA_ADMIN_UI_BOOTSTRAP", "127.0.0.1:9092")
        .env("KRABKA_ADMIN_UI_SECURITY_PROTOCOL", "SASL_SSL")
        .env("KRABKA_ADMIN_UI_TLS_SERVER_NAME", " \t ")
        .env_remove("KRABKA_ADMIN_UI_LISTEN_ADDR")
...
fn whitespace_tls_server_name_from_env_child() {
    if std::env::var_os("KRABKA_ADMIN_UI_CONFIG_WHITESPACE_TLS_CHILD").is_none() {
        return;
    }
...
fn from_env_rejects_tls_client_cert_without_key() {
    let output = Command::new(std::env::current_exe().expect("test binary path is available"))
        ...
        .arg("--nocapture")
        .env("KRABKA_ADMIN_UI_CONFIG_INCOMPLETE_TLS_IDENTITY_CHILD", "1")
        .env("KRABKA_ADMIN_UI_BOOTSTRAP", "127.0.0.1:9092")
        .env("KRABKA_ADMIN_UI_SECURITY_PROTOCOL", "SASL_SSL")
        .env("KRABKA_ADMIN_UI_TLS_SERVER_NAME", "localhost")
        .env("KRABKA_ADMIN_UI_TLS_CLIENT_CERT_PEM", "client.crt")
        .env_remove("KRABKA_ADMIN_UI_TLS_CLIENT_KEY_PEM")
        .env_remove("KRABKA_ADMIN_UI_LISTEN_ADDR")
        .output()
        .expect("child test process runs");
...
fn from_env_rejects_tls_client_key_without_cert() {
    let output = Command::new(std::env::current_exe().expect("test binary path is available"))
        ...
        .env("KRABKA_ADMIN_UI_SECURITY_PROTOCOL", "SASL_SSL")
        .env("KRABKA_ADMIN_UI_TLS_SERVER_NAME", "localhost")
        .env_remove("KRABKA_ADMIN_UI_TLS_CLIENT_CERT_PEM")
        .env("KRABKA_ADMIN_UI_TLS_CLIENT_KEY_PEM", "client.key")
        .env_remove("KRABKA_ADMIN_UI_LISTEN_ADDR")
        .output()
        .expect("child test process runs");
...
fn incomplete_tls_client_identity_from_env_child() {
    if std::env::var_os("KRABKA_ADMIN_UI_CONFIG_INCOMPLETE_TLS_IDENTITY_CHILD").is_none() {
        return;
    }
