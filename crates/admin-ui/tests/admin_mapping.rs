...
fn kafka_error_dto_from_kafka_error_preserves_fields() {
    ...
    assert!(dto.code == 36);
    assert!(dto.name == "TOPIC_ALREADY_EXISTS");
    assert!(dto.message == Some("topic exists".to_string()));
}
...
fn maps_topic_metadata_to_rows_with_errors() {
    ...
    assert!(rows[0].topic_id == Some(topic_id.to_string()));
...
fn maps_group_ids_to_group_rows() {
    ...
    assert!(rows.len() == 2);
    assert!(rows[0].group_id == "payments");
    assert!(rows[1].group_id == "shipping");
}
...
fn maps_user_quota_config_to_visible_rows() {
    ...
    assert!(rows[0].quota_type == "consumer_byte_rate");
    assert!(rows[0].value == "2048");
    assert!(rows[1].quota_type == "producer_byte_rate");
    assert!(rows[1].value == "1024.5");
}

#[test]
fn maps_scram_credentials_to_user_rows() {
    let rows = krabka_admin_ui::admin::user_rows(vec![UserScramCredentials {
        username: "alice".to_string(),
    ...
    assert!(rows[0].principal == "SCRAM-SHA-512");
}
...
fn maps_mutation_outcomes_preserving_kafka_errors() {
    ...
    assert!(delete_topic_rows[0].resource == "orders");
    assert!(delete_topic_rows[0].has_error());
    assert!(partition_rows[0].resource == "payments");
    assert!(scram_rows[0].resource == "alice");
    assert!(log_dir_rows[0].resource == "orders-1");
